...
- 本地 Key 必须放在实际运行项目的本地配置文件中（通常是项目根目录 `.env.local`），不能假定 Skill 仓库或全局 Skill 目录的 `.env.local` 会自动被外部项目读取；必须在项目 README 中写清楚实际路径。
...
- 当目标是可运行的 Web 项目时，默认同时配置本地后端代理或接入项目已有后端：由后端读取项目 `.env.local` 中的本地 Key，前端只能通过本机 API 请求模型列表和连接测试；不能把只完成 UI Mock 当作接入完成。开发启动命令应默认同时启动前端和本地代理，或明确提供等价的双进程启动方式。
...
6. 将模型分成三层：服务商公开目录、当前 Key 已验证模型、项目默认模型。公开目录不等于当前 Key 可用，项目默认模型也必须允许被当前项目覆盖。
   - 模型发现成功后，以服务商真实返回的模型 ID 替换当前服务商的预设候选；只做完全相同字符串去重，不要擅自把相似名称推断为别名。服务商若明确返回 alias/替代关系，才可以展示为别名。
7. 只有在没有历史验证记录、用户主动刷新、Endpoint 变化或历史模型连接失败时，才执行模型发现流程：
   - 服务商支持模型列表接口时，读取结构化模型列表；
   - 不支持或列表不可用时，使用注册表中的候选模型逐个做低成本连接测试；
   - 将结果保存为结构化记录，而不是把服务商返回的原始文本直接放入下拉框。
   - 没有真实 Key、后端代理或成功的服务商响应时，必须明确返回“无法获取真实模型列表”；演示数据只能标记为 `mock` 或“预设候选”，不能伪造 `discovered`、`verified` 或成功结果。
   - 发现成功后保存非敏感的模型 ID、服务商、Endpoint、Key 来源和验证时间；普通页面刷新优先使用这份记录，不重复请求服务商。
8. UI 提供“获取 / 刷新可用模型”操作。首次进入优先显示项目默认模型和历史验证模型；页面普通刷新不自动请求服务商。用户主动刷新、服务商变化、Key 变化或模型失效时才重新获取。
9. UI 中将已验证模型、发现但未验证模型、连接失败模型、项目默认模型和手动输入模型分开显示。手动模型应通过“其他模型 / 手动输入模型”进入，并在首次使用前测试。
10. 使用统一的连接测试请求验证 API Key、Endpoint 和模型，返回结构化状态：`ok`、`provider`、`model`、`key_source`、`checked_at`、`error_kind`、`retryable`、`message`。
...
13. 对每个已配置本地 Key 的服务商至少执行一次模型列表请求和默认模型连接测试；默认模型必须是该 Key 实际返回并验证通过的 ID。没有 Key 的服务商必须标记为“未配置/未验证”，不能宣称默认模型可用。
...
“获取 / 刷新可用模型”必须展示结构化结果和可操作动作：用户可以将模型加入项目默认列表、查看失败原因、重试单个模型，或把模型复制到手动输入框；不能只展示一段不可操作的文本。模型目录和验证结果建议按项目本地保存，不能把某个项目的 Key 权限写成所有项目的全局事实。
